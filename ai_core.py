...
from talking_brain.talking_brain_connector import TalkingBrainConnector
...
class VALECore:
    ...
    def __init__(self):
        ...
        # Talking Brain
        #
        # This is a communication layer only. It does not replace the
        # specialized VALE brains or participate in their selection.
        # If it is unavailable, the existing VALE response pipeline
        # continues normally.
        # --------------------------------------------------------------

        self.talking_brain = TalkingBrainConnector()

        # --------------------------------------------------------------
        # Talking Brain diagnostics
        # --------------------------------------------------------------
        # Keep the application alive if the model cannot load, but expose
        # the exact initialization state in Render logs instead of silently
        # hiding the reason.
        try:
            tb_status = self.talking_brain.status()
        ...
        print(
            "\n============================================================\n"
            "🧠 VALE TALKING BRAIN INITIALIZATION\n"
            f"{tb_status}\n"
            "============================================================",
            flush=True,
        )
    ...
    def process(
        ...
        print(
            f"🧠 VALE CORE PROCESS START | message={original_message[:200]!r}",
            flush=True,
        )
        ...
        # --------------------------------------------------------------
        # Talking Brain receives the actual results produced by the
        # existing VALE brains and expresses them in natural English.
        #
        # IMPORTANT: this is intentionally non-destructive. If Talking
        # Brain is unavailable, fails, or has no usable input, VALE falls
        # back to its original response extraction below.
        # --------------------------------------------------------------
        ...
        try:
            ...
            brain_results = network_result.get(
                "results",
                {},
            )
            ...
            print(
                "🧠 TALKING BRAIN INPUT | "
                f"selected={network_result.get('selected_brains', [])} "
                f"result_keys={list(brain_results.keys()) if isinstance(brain_results, dict) else type(brain_results).__name__}",
                flush=True,
            )

            def _brain_text(name: str) -> str:
                entry = (
                    brain_results.get(name)
            ...
            talking_response = self.talking_brain.chat(
                message=original_message,
                unity=_brain_text("UNITY"),
                heroic=_brain_text("HEROIC"),
            )

            if talking_response:
                brain_response = talking_response
                print(
                    "✅ TALKING BRAIN RESPONSE GENERATED",
                    flush=True,
                )
            else:
                print(
                    "⚠️ TALKING BRAIN RETURNED NO RESPONSE",
                    flush=True,
                )

        except Exception as exc:
            # Talking Brain must never break the existing VALE pipeline.
            # Log the real exception so Render exposes the root cause.
            print(
        ...
        # --------------------------------------------------------------
        # Existing VALE response extraction remains the fallback.
        # --------------------------------------------------------------
        ...
        if brain_response:
            ...
            print(
                "🗣️ VALE RESPONSE READY | source=brain_network",
                flush=True,
            )
