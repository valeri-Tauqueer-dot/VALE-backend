...
"""Raised when uploaded media is invalid or unsupported."""
...
"VALE_MAX_IMAGE_BYTES",
10 * 1024 * 1024,
),
max_audio_bytes=_env_int(
"VALE_MAX_AUDIO_BYTES",
30 * 1024 * 1024,
),
max_document_bytes=_env_int(
"VALE_MAX_DOCUMENT_BYTES",
30 * 1024 * 1024,
),
...
Browser-provided MIME types are useful but not fully trusted.
"""
...
"""
Route the upload to the correct processor.
"""
...
content_type: Optional[str],
limits: MediaLimits = DEFAULT_LIMITS,
) -> tuple[str, str, MediaType]:
"""
Validate basic upload properties.
...
if media_type == MediaType.UNKNOWN:
    raise MediaValidationError(
        ...
        "Supported types are images, audio, PDF, TXT and DOCX."
    )
...
if len(data) > max_size:
    ...
    raise MediaValidationError(
        f"File is too large. "
        f"Maximum size for {media_type.value} "
        f"is {max_mb:.0f} MB."
    )
...
This catches many corrupted or falsely labelled uploads.
"""
