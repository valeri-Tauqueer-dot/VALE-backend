...
class MediaError(Exception):
    """Base exception for VALE media errors."""


@dataclass
class MediaResult:
    """Stable, JSON-safe result shared by every media processor."""
    ...
    def build_brain_context(self, max_text_chars: int = 50_000) -> str:
        parts = [
            "=== VALE MEDIA CONTEXT ===",
        ...
        if self.description:
            parts += ["", "MEDIA UNDERSTANDING:", self.description]

        if self.extracted_text:
            text = self.extracted_text.strip()
            if len(text) > max_text_chars:
                text = text[:max_text_chars] + "\n\n[Media text truncated.]"
        ...
        if self.warnings:
            parts += ["", "PROCESSING WARNINGS:"]
            parts.extend(f"- {item}" for item in self.warnings)

        if self.errors:
            parts += ["", "PROCESSING ERRORS:"]
